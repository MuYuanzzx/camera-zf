...
#ifndef __DRV_NOTIFY_H
#define __DRV_NOTIFY_H
...
typedef struct
{
  uint32_t duration;	 		//指示灯持续时间
  uint32_t period;				//指示灯呼吸周期
  float cycle_period;  //循环周期
  uint32_t free_time;     //空闲时间
  rgb_mode mode;
  uint32_t start_time;
  uint8_t start_flag;
  uint32_t cnt;
